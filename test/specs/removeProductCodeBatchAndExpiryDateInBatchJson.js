...
describe('127_Update a batch via import of Json by deleting product code, batch and expiry date', () => {
    ...
    it('Browser - update a batch via import of Json ', async () => {
        ...
        // await batches.invalidFieldInfoRequired(["productCode - Required field, batch - Required field", "expiryDate - Required field"])
        // await wait.setTimeoutwait(5);
        ...
        try {
            ...
            await wait.setTimeoutwait(5);
            expect(JSON.stringify(e)).to.equal(0, `${testData.json.productCode}`, `${testData.json.batch}`, `${testData.json.expiryDate} not found in failed logs`)
