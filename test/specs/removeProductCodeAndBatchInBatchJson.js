...
describe('124_Update a batch via import of Json by deleting product code and batch element', () => {



    it('Browser - update a batch via import of Json ', async () => {
        allureReporter.addTestId('ImportJson_4_4')
        allureReporter.addDescription('Update a batch via import of Json by deleting product code and batch element and upload modified file. View message and click on invalid field info')
        allureReporter.addStep('1. Use the standard template Json')
        allureReporter.addStep('2. Delete product code and batch element in the json')
        ...
        // await batches.invalidFieldInfoRequired(["productCode - Required field", "batch - Required field"])
        // await wait.setTimeoutwait(5);

        try {
            const productCode = await batches.firstRow()
            await wait.setTimeoutwait(5);
            const batch = await batches.secondRow()
            await wait.setTimeoutwait(5);
            ...
            await wait.setTimeoutwait(5);
            expect(JSON.stringify(e)).to.equal(0, `${testData.json.productCode, testData.json.batch}`, `not found in failed logs`)
