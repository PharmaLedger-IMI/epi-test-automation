...
describe('Batch Recall and Recall Message for Non-serialized batches ', () => {
    ...
    it('Batch recall for non-serialized_2-should verify Batch Recall and Recall Message for Non-serialized batches ', async () => {
       ...
       // allureReporter.startStep('Go back to the Batch on the Enterprise Wallet and undo the batch recall flag. ')
        ...
        await data.generateExpectationFile(info.getProductId(), info.getbatchId(true), info.getCurrentRandomDate(),  "",info.getBrandName(),info.getBatchRecall(), await batches.checkBatchMessage(),"", await batches.checkBatchRecallMessage() )
        ...
        matrix.generate2dMatrix(info.getProductId(), info.getbatchId(true), info.getCurrentRandomDate())
