...
describe('015_Edit batch and enable expired expiry date check with valid expiry date ', () => {
    ...
    if (!process.env.npm_config_browserOnly) {
        ...
        after(async () => {
            ...
            await utilityFunction.runAppium("enableExpiredFlagWithValidExpiryDateTestRun")
        })
    ...
    it('Browser - should verify that the expired expiry date check is enabled by default', async () => {
        allureReporter.addDescription('Edit batch and verify that the expired expiry date check is enabled. Pass valid expired date in matrix')
        allureReporter.addStep(' Verify that the expired expiry date check is enabled by default in batch')
        allureReporter.addStep(' Scan a data matrix code with expiry date')
        allureReporter.addTestId('BasicAuthFeatureTest_2_4')
        //click batch
