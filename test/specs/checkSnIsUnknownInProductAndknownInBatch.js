...
describe('088_Edit product to check SN is unknown and edit batch to update valid SN', () => {
    ...
    if (!process.env.npm_config_browserOnly) {
        ...
        after(async () => {
            ...
            const { stdout1, stderr1 } = await exec('cd ../epi-mobileapp-test-automation && npx kill-port 4723 && npm run checkTheSNIsUnknownInProductAndKnownInBatchTest');
    ...
    it('Browser - should check Serial number is known', async () => {
        allureReporter.addStep("Check SN is Unknown flag in product")
        allureReporter.addStep("Check serial number is known in batch")
