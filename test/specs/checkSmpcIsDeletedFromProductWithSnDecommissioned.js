...
describe('084_Edit product to check SN is decommissioned and delete smpc. Edit batch to set decommissioned serial number', () => {
    ...
    if (!process.env.npm_config_browserOnly) {
        ...
        after(async () => {
            ...
            const { stdout1, stderr1 } = await exec('cd ../epi-mobileapp-test-automation && npx kill-port 4723 && npm run checkTheSmpcDeletedFromProductWithSNDecommissonedTest');
    ...
    it('Browser - should check If SMPC is deleted from the product and the serial number on the barcode is decommissioned', async () => {
